...
def send_query_to_openai(user_prompt, system_prompt, model_name="gpt-3.5-turbo", temperature=0.7) -> str:
    """
    Send a query to OpenAI API and get the response
    Single thread for LLM-based anomaly generation
    """
...
def send_query(user_prompt, system_prompt, model_name, api_key, temperature) -> str:
    ...
    if "qwen" in model_name.lower() or "gemini" in model_name.lower():
...
def inference_on_openai(testing_data: List[Dict], system_prompt: str, model_name: str, api_key: str, temperature: float) -> List[Dict]:
    """
    Inference on the openai api, with parallel threads
...
if __name__ == "__main__":
    # test example for parallel API calling
    user_prompt = "What is the capital of France?"
    system_prompt = "You are a helpful assistant."
    response = send_query_to_openai(user_prompt, system_prompt, model_name="gpt-3.5-turbo", temperature=0.7)
